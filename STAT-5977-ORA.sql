...
--  ** ***************************************************************
--  **
--  **  Name:        STAT-5977-ORA.sql (Make oauth2.0 client secret optional in win client.)
--  **
--  **  Description: Make oauth2.0 client secret optional in win client. 
...
--  **  Script Summary:
--  **  
--  **  a. Alter column CLIENT_SECRET for making CLIENT SECRET optional in table OAUTH2_CLIENT_DETAILS in stat 6.3
--  **  
...
SET LINESIZE 999
SPOOL [SPOOLPT]/STAT-5977-ORA.LOG


SET SERVEROUTPUT ON SIZE 1000000
DECLARE
   l_script     VARCHAR2 (500 CHAR)   := 'STAT-5977-ORA.sql Started';
...
DECLARE
    AlterDone NUMBER;
BEGIN
   SELECT COUNT(*) INTO AlterDone FROM USER_TAB_COLUMNS WHERE TABLE_NAME='OAUTH2_CLIENT_DETAILS' AND COLUMN_NAME in ('CLIENT_SECRET') and NULLABLE = 'N';
    IF AlterDone > 0 THEN
        execute immediate ('Alter table OAUTH2_CLIENT_DETAILS modify CLIENT_SECRET VARCHAR2(256 CHAR) NULL');
         DBMS_OUTPUT.put_line('*   OAUTH2_CLIENT_DETAILS - New column CLIENT_SECRET added');
    END IF;
END;
/
...
SET SERVEROUTPUT ON SIZE 1000000
DECLARE
   l_script     VARCHAR2 (500 CHAR)   := 'STAT-5977-ORA.sql Completed';
   l_username   VARCHAR2 (30 CHAR);
   l_schemaname VARCHAR2 (30 CHAR);
   l_machine    VARCHAR2 (64 CHAR);
   l_osuser     VARCHAR2 (30 CHAR);
...
COMMIT;
PROMPT ***
 SELECT '* End of STAT-5977-ORA-OracleServer for '||COMPANY_NAME FROM STAT.SYSTEM_TBL;
PROMPT *** ****************
